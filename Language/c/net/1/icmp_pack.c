...
void SendPacket(int sockfd, struct sockaddr_in *dest_addr, int nSend)
{
    SetICMP(nSend);
    if (sendto(sockfd, SendBuffer, packageLen, 0,
...
void Statistics(int signo)
{
    ...
    printf("%d packets transmitted, %d received, %d%% packet loss, time %dms\n", nSend, nRecv, (nSend - nRecv) / nSend * 100, (int)GetRtt(&LastRecvTime, &FirstSendTime));
...
int RecvePacket(int sockfd, struct sockaddr_in *dest_addr)
{
    int RecvBytes = 0;
