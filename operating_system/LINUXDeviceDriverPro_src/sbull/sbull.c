...
static void sbull_request(request_queue_t *q)
{
   ...
   while ((req = elv_next_request(q)) != NULL)//elv_next_request：获取队列中第一个未完成的请求。没有请求时返回null.
   {
      struct sbull_dev *dev = req->rq_disk->private_data;
      /**
       * blk_fs_request：告诉用户该请求是否是一个文件系统请求-移动块数据的请求。
