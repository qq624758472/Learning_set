...
//arm-himix200-linux-gcc -o ftp ftp.c
/*FTP Client*/
...
char *ntpdate(char *DEVID);

/////////////// main ///////////////////////////////
char TIME_DIRNAME[32] = {0};
char COMPANYID[16] = {0};
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2)
	{
		printf("Usage: %s <ip address> <imgName>\n", argv[0]);
		exit(1);
	}

	//获取日期
	memset(TIME_DIRNAME, 0, 32);
	char *nowTime = ntpdate("JTJL99996");
	for (int i = 0; i < 8; i++)
	{
		TIME_DIRNAME[i] = nowTime[10 + i];
	}
	//printf("TIME_DIRNAME:%s\n",TIME_DIRNAME);
	free(nowTime);

	memcpy(COMPANYID,argv[3] ,strlen(argv[3]));
...
void session_create(int sock, char *ip)
{
	...
	*/
	...
	memset(buf, 0, sizeof buf);
	//scanf("%s", info);
	memcpy(info, "1q2w3e4r5T", strlen("1q2w3e4r5T"));
...
void get(char *file, const int sd, char *ip)
{
	...
	if (0 > (fd = open(file, O_CREAT | O_TRUNC | O_WRONLY, S_IRUSR | S_IWUSR)))
	{ //read/write
...
void service_handleCmd(const int socket, char **ap_argv, const int a_argc, char *ip)
{
	...
	else if (strcmp(ap_argv[0], "chmod") == 0)
	{
		if (a_argc == 1)
			ls_non_path(ap_argv[1], socket, ip);
		else
	...
	else if (strcmp(ap_argv[0], "stor") == 0)
	{
		if (a_argc == 1)
			ls_non_path(ap_argv[1], socket, ip);
		else
	...
	else if (strcmp(ap_argv[0], "passive") == 0)
	{
		if (a_argc == 1)
			ls_non_path(ap_argv[1], socket, ip);
		else
...
void service_create(int socket, char *ip, char *imgName)
{
	...
	//const char* COMPANYID = "1000091";  //公司ID 。
	//const char* TIME_DIRNAME = "20200319"; //以日期命名的目录
	//const char* imgName = "JTJL99996_20200316144444.jpg";

	//sprintf(cmd1,"cd alarm;mkdir %s;cd %s;mkdir pic;chmod 777 pic;cd pic;mkdir %s;chmod 777 %s;cd %s;put %s;chmod 777 %s;quit;",COMPANYID,COMPANYID,TIME_DIRNAME,TIME_DIRNAME,TIME_DIRNAME,imgName,imgName);
	for (;;)
	{
		//__fpurge(stdin);  //清空输入缓冲区。
		printf("ftp> ");
		//__fpurge(stdin);
		memset(&cmd, '\0', sizeof(cmd)); // clear buffer
		//fgets(cmd, sizeof(cmd), stdin);
		...
		{
		case 0:
			memcpy(cmd, "cd alarm", strlen("cd alarm"));
			i++;
			break;
		case 1:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "mkdir %s", COMPANYID);
			//memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 2:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "cd %s", COMPANYID);
			memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 3:
			memcpy(cmd, "mkdir pic", strlen("mkdir pic"));
			i++;
			break;
		case 4:
			//memcpy(cmd, "chmod 777 pic", strlen("chmod 777 pic"));
			i++;
			break;
		case 5:
			memcpy(cmd, "cd pic", strlen("cd pic"));
			i++;
			break;
		case 6:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "mkdir %s", TIME_DIRNAME);
			//memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 7:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "chmod 777 %s", TIME_DIRNAME);
			//memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 8:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "cd %s", TIME_DIRNAME);
			memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 9:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "put %s", imgName);
			memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 10:
			memset(cmd1, 0, sizeof(cmd1));
			sprintf(cmd1, "chmod 777 %s", imgName);
			memcpy(cmd, cmd1, strlen(cmd1));
			i++;
			break;
		case 11:
			memcpy(cmd, "exit", strlen("exit"));
			i++;
			break;
		default:
			printf("CMD Error!\n");
...
char *ntpdate(char *DEVID)
{
	...
	char *y = time_tran(now_time.tm_mon + 1);
	char *r = time_tran(now_time.tm_mday);
	char *s = time_tran(now_time.tm_hour);
	char *f = time_tran(now_time.tm_min);
	char *m = time_tran(now_time.tm_sec);

	//printf("ive_ntp_get_now_time(): n:%d y:%s r:%s s:%s f:%s m:%s   buf:%s\n",now_time.tm_year +1900, y, r, s,f ,m , buf);
	sprintf(buf, "%s_%d%s%s%s%s%s", DEVID, now_time.tm_year + 1900, y, r, s, f, m);
